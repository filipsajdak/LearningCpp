...
class Logger
{
  public:
    template <typename T>
    Logger(T *logger)
        : logger(logger), log_fun([](void *l, const std::string &msg) {
...
class UsersDataBaseTypeErased
{
  public:
    ...
    UsersDataBaseTypeErased(T *users)
        : users(users), findUser_fun([](void *u, const std::string &login) {
              return reinterpret_cast<T *>(u)->findUser(login);
          })
    {
        // empty
    }
    ...
    {
        return findUser_fun(users, login);
    }

  private:
    void *users;
    const User *(*findUser_fun)(void *, const std::string &);
};
...
class LoggerMock
{ // Please notice that it is not inheriting any interface or baseclass
  public:
    MOCK_METHOD1(log, void(const std::string &));
};


TEST(AuthorizationTests_type_erased, correct_login)
{
...
TEST(AuthorizationTests_type_erased, incorrect_login)
{
    User user{"login", "password"};
    UsersDataBaseMock users;
    LoggerMock logger;
